...
There were problems with type conversions from commutative rings 
to non-unital commutative semigroups in Lean. 
Therefore, this files uses commutative rings instrad of commutative semigroups.
-/
...
def is_rel_inv {A : Type} [comm_ring A] (a b : A) :=
-- let e := (a * b) in 
...
def all_rel_inv (A : Type*) [comm_ring A] : Prop := 
∀ (a : A), ∃ (b : A), is_rel_inv a b

lemma rel_inv_comm {A : Type} [comm_ring A] {a b : A} : is_rel_inv a b → is_rel_inv b a :=
begin
  unfold is_rel_inv, 
...
lemma rel_inv_is_unique {A : Type} [comm_ring A] (a b b' : A)
...
lemma inv_invariant_hom_a_b {A : Type} [comm_ring A] {A' : Type} [comm_ring A'] 
...
lemma inv_invariant_a_b {A : Type} [comm_ring A] {A' : Type} [comm_ring A'] 
(h : A ≃* A') { b : A} {a' : A'}: 
...
lemma rel_inv_invariant_isomorphism' {A : Type} [comm_ring A] {A' : Type} [comm_ring A'] 
...
lemma product_all_rel_inv {A : Type*} [comm_ring A] {A' : Type*} [comm_ring A']  
